...
var methods = {
	...
	nextFeedDownloaded : function(){
		var it = window.reader.retreiveNewFeed();
	...
	requestMore : function(){
		if (!info.waitingMore && !info.noMore){
			...
			var cs = data[info.curMajorIndex].continuation;
			if (cs == null)
				cs = "";
			
			touchView.postData({
				type:"more",
				continuationString:cs
			});
	...
	setItemNumber : function(id){
		...
		touchView.postData({
				type: "info",
				feed: currentPoint.item.origin.streamId,
				item: currentPoint.item.id
			});
		loadDiv('item' + id);
		log("item" + id);
	},
